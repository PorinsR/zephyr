...
static int spi_sam_configure(const struct device *dev, const struct spi_config *config)
{
	...
	if (SPI_WORD_SIZE_GET(config->operation) == 8) {
		...
		spi_csr |= SPI_CSR_BITS(SPI_CSR_BITS_9_BIT);
	} else {
		return -ENOTSUP;
	}
...
#define SPI_SAM_RTIO_DEFINE(n)                                                                     \
	SPI_RTIO_DEFINE(spi_sam_rtio_##n, CONFIG_SPI_SAM_RTIO_SQ_SIZE, CONFIG_SPI_SAM_RTIO_SQ_SIZE)
...
#define SPI_SAM_DEVICE_INIT(n)                                                                     \
	...
	SPI_DEVICE_DT_INST_DEFINE(n, &spi_sam_init, NULL, &spi_sam_dev_data_##n,                   \
				  &spi_sam_config_##n, POST_KERNEL, CONFIG_SPI_INIT_PRIORITY,      \
				  &spi_sam_driver_api);
